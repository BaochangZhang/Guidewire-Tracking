...
from utils.general import check_img_size, check_requirements, non_max_suppression_detection, apply_classifier, \
    scale_coords, xyxy2xywh, set_logging, increment_path, refind,refind_Rui
...
def detect(save_img=True):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Apply NMS
        pred_high, pred_candidates,_ = non_max_suppression_detection(prediction=pred, conf_factor=opt.conf_factor, iou_thres = opt.iou_thres, merge_thres=0.5,merge=False, possible_targer_n=5, wh_range=[20, imgsz], classes=opt.classes, agnostic=opt.agnostic_nms)
        ...
        Target,Target_candidate = refind_Rui(Target,Target_candidate,pred_high,pred_candidates, frame, conj_thres=0.2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', default='runs/mydetect_test/all_exp_yolov5s_det', help='save results to project/name')
    parser.add_argument('--name', default='exp_Rui', help='save results to project/name')
